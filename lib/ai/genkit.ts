...
export async function streamWithKeyRotation<T>(
  ...
  while (attempts < maxRetries) {
    ...
    try {
      ...
      if (isRateLimit && currentKey) {
        console.warn(`Genkit streaming operation failed with rate limit for key ending in ...${currentKey.slice(-4)}, retrying...`);
        apiKeyManager.markKeyFailed(currentKey);
        continue;
      }
  ...
  throw new Error('Max retries exceeded for Genkit streaming operation');
}
